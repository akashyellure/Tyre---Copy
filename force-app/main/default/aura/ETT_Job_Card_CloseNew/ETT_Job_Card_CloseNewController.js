({
    ...
    handleSubmit : function(component, event, helper) {
        ...
        let hasError = false;
        console.log('enterLines==='+JSON.stringify(component.get("v.newJobCardCloseLines")))
        var newJobCardCloseLines=component.get('v.newJobCardCloseLines');
        var allfalse = jsonValues.some(function(value) {
          return value === true;
        });
        
        newJobCardCloseLines.forEach(function(item){
            ...
            if(item.Issued_Quantity__c > item.Factory_Stock__c){
                hasError = true;
                var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
                "type": "error",
                "message": "Issue quantity should not be more than factory stock"
            });
            toastEvent.fire();
            }
        ...
        if (allfalse===false && jobinfo.Customer_Type__c == 'Supplier'&& (prodstatus === 'Send Back'||prodstatus === 'Scrapped')) {
            // Display an error message
            hasError = true;
            var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
                "type": "error",
                "message": "Rejection Analysis Should not be blank, Please Select any one Field"
            });
            toastEvent.fire();
            /* var inputField = component.find("v.prodstatus");
            inputField.set("v.errors", [{message:"if party type is internal then dont select send back"}]);
            return false;*/
}
            ...
            toastEvent.setParams({
                "type": "error",
                "message": "Status should not be Send Back for Internal party type"
            });
            toastEvent.fire();
            /* var inputField = component.find("v.prodstatus");
            inputField.set("v.errors", [{message:"if party type is internal then dont select send back"}]);
